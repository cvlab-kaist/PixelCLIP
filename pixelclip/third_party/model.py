...
class Attention(nn.MultiheadAttention):
    ...
    #@torch.backends.cuda.sdp_kernel(enable_math=True, enable_flash=True, enable_mem_efficient=False)
...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward_mask(self, x: torch.Tensor, mask: torch.Tensor):
        _x = x.repeat(1, mask.shape[0], 1)
        attn_mask = torch.zeros(mask.shape[0], x.shape[0], x.shape[0], device=mask.device)
        attn_mask[:, 0, 0] = 1
        attn_mask[:, 0, 1:] = mask.flatten(1)
        attn_mask[:, 1:, 0] = mask.flatten(1)
        #attn_mask = ~(attn_mask.bool())
        attn_mask = attn_mask * 10
        attn_mask = torch.repeat_interleave(attn_mask, self.attn.num_heads, dim=0)

        att = self.attn(_x, _x, _x, need_weights=False, attn_mask=attn_mask)[0]
        x = _x + att
        x = x + self.mlp(self.ln_2(x))
        return x
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, dense=False, mask=None, prompt=None):
        for i, resblock in enumerate(self.resblocks):
            ...
            elif i == self.layers -1 and mask is not None:
                x = resblock.forward_mask(x, mask)
            else:
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, dense=False, mask=None):
        ...
        x = self.transformer(x, dense, mask)
