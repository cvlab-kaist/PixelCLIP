...
def register_all_coco_stuff_10k(root):
    ...
    for name, image_dirname, sem_seg_dirname in [
        ...
        ("semi", "val2017", "annotations_detectron2/masks"),
    ]:
...
def load_sam_instances(root, sam_root):
    """
    ...
    """
    dicts = []
    coco_dir = os.path.join(root, "coco/train2017")
    coco_images = os.listdir(coco_dir)
    sam_dir = os.path.join(root, sam_root)
    for c in coco_images:
        fn = c.split(".")[0] + '.json'
        dicts.append({'img_name': os.path.join("coco/train2017", c), 
                  "ann_name": os.path.join(sam_root, fn)})
    
    return dicts

def register_all_coco_stuff_sam(root):
    #root = os.path.join(root, "coco")
    _sam_root = os.getenv("COCO_SAM", "../../hdd/hz/semiseg/coco/sam_mask")
    meta = _get_coco_stuff_meta()
    for name, image_dirname, sem_seg_dirname in [
        ("train", "train2017", "../../../hdd/hz/semiseg/coco/sam_mask"),
    ]:
        #image_dir = os.path.join(root, image_dirname)
        #gt_dir = os.path.join(root, sem_seg_dirname)
        name = f"coco_sam_{name}_stuff_all_sem_seg"
        # DatasetCatalog.register(
        #     name, lambda x=image_dir, y=gt_dir: load_sem_seg(y, x, gt_ext="png", image_ext="jpg")
        # )
        DatasetCatalog.register("{}".format(name), lambda: load_sam_instances(root, _sam_root))
        MetadataCatalog.get(name).set(
            dirname=root,
...
register_all_coco_stuff_sam(_root)
